...
set FFMPEG_INSTALL_DIR=%BASE%\onvif-gui-win-libs\ffmpeg
set SDL2_INSTALL_DIR=%BASE%\onvif-gui-win-libs\sdl
...
for %%v in %list% do (
    ...
    delvewheel repair dist\*cp%%v-cp%%v-*.whl --add-path %BASE%\onvif-gui-win-libs\ffmpeg\bin --add-path %BASE%\onvif-gui-win-libs\sdl\bin
