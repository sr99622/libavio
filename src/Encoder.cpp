    ...
    try {
        ...
        if (frame) {
            if (mediaType == AVMEDIA_TYPE_VIDEO && !cmpFrame(frame)) {
                std::cout << "start video convert" << std::endl;
                if (!sws_ctx) {
                    std::cout << "CREATE VIDEO FRAME CONVERTER" << std::endl;
                    ...
                    std::cout << "sws_ctx created" << std::endl;
                }
                ...
                frame = cvt_frame;
                //Frame qf(cvt_frame);
                //std::cout << qf.description() << std::endl;
                std::cout << "finish video convert" << std::endl;
            }
            ...
            if (hw_device_type != AV_HWDEVICE_TYPE_NONE) {
                ...
                //Frame fuck(frame);
                //std::cout << "FUCK: " << fuck.description() << std::endl;
...
void Encoder::showParameters()
{
    if (mediaType == AVMEDIA_TYPE_VIDEO) {
        const char* pix_fmt_name = "unknown pixel format";
        const char* name = av_get_pix_fmt_name((AVPixelFormat)enc_ctx->pix_fmt);
        if (name)
            pix_fmt_name = name;

        const char* codec_name = "unknown codec";
        const char* name2 = avcodec_get_name(enc_ctx->codec_id);
        if (name2)
            codec_name = name2;
        

        std::stringstream str;
        str << "Video Encoder Parameters"
            << "\ncodec_id: " << codec_name
            << "\nbit_rate: " << enc_ctx->bit_rate
            << "\nwidth: " << enc_ctx->width
            << "\nheight: " << enc_ctx->height
            << "\ntime_base num: " << enc_ctx->time_base.num
            << "\ntime_base den: " << enc_ctx->time_base.den
            << "\ngop size: " << enc_ctx->gop_size
    ...
    if (mediaType == AVMEDIA_TYPE_AUDIO) {
        /*
        std::cout <<
            "\nin_channel_count    " << frame->channels <<
            "\nout_channel_count   " << channels <<
            "\nin_channel_layout   " << frame->channel_layout <<
            "\nout_channel_layout  " << channel_layout <<
            "\nin_sample_rate      " << frame->sample_rate <<
            "\nout_sample_rate     " << sample_rate <<
            "\nin_sample_fmt       " << (AVSampleFormat)frame->format <<
            "\nout_sample_fmt      " << sample_fmt << std::endl;
        */
    }
}
